...
async def initialize_database():
    ...
    async with postgres_pool.acquire() as conn:
        ...
        await conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_users_house ON users(house)
        """)
    ...
    try:
        ...
        async with postgres_pool.acquire() as conn:
            ...
            if not user:
                # New user
                await conn.execute("""
                    INSERT INTO users (user_id, username, house, total_points, problems_solved, submissions)
                    VALUES ($1, $2, $3, $4, $5, $6)
            ...
            # Record submission
            await conn.execute("""
                INSERT INTO submissions (user_id, problem_id, language, verdict, points)
                VALUES ($1, $2, $3, $4, $5)
        ...
        async with postgres_pool.acquire() as conn:
            houses = await conn.fetch("""
                SELECT 
                    ...
                    total_points,
                    members,
                    ROUND(CAST(total_points AS FLOAT) / NULLIF(members, 0), 2) as avg_points_per_member,
